...
input <- austinHousingData2[,c("variable","variable")]
model <- lm(latestPrice~propertyTaxRate+garageSpaces+hasCooling, data = input)

```

```{r}
# some replicable function that takes in variables from multivariate regression above with default values set to random distribution output, and returns price
```
